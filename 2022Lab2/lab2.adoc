...
Класс LedTask должен ожидать прихода события EventButtonPress и если оно пришло, то переключить светодиод.

Опишем конструктор класса LedTask, используя Event и переопределим метод Execute.
