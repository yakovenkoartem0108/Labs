...
{
public: 
  void Execute() override; // pereopredelyaem virtyal metod
};
...
[source, cpp]
#include "rtos.hpp"         // for Rtos Dlya podklychenia OC
...
OsWrapper::Event event{500ms, 1};
MyTask myTask(event, UserButton::GetInstance()); // sosdali obiekt atogo classa
